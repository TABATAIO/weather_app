...
@section('title', '第四形態進化編集')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">第四形態進化編集</h1>
        ...
        <form action="{{ route('admin.fourth-form-evolutions.update', $fourthFormEvolution) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" 
                           id="name" 
                           name="name" 
                           value="{{ old('name', $fourthFormEvolution->name) }}"
                           required
                ...
                <!-- 現在の画像表示 -->
                @if($fourthFormEvolution->image_path)
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">現在の画像</label>
                        <div class="flex items-center space-x-4">
                            <img src="{{ asset('storage/' . $fourthFormEvolution->image_path) }}" 
                                 alt="{{ $fourthFormEvolution->name }}" 
                                 class="w-24 h-24 rounded-lg object-cover border">
                            <span class="text-sm text-gray-600">新しい画像をアップロードすると、この画像は置き換えられます</span>
                        </div>
                    </div>
                @endif
                ...
                <div class="mb-4">
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                        新しい画像 {{ $fourthFormEvolution->image_path ? '(変更する場合のみ)' : '' }}
                    </label>
                    <input type="file" 
                           id="image" 
                           name="image" 
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" 
                              ...
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                              placeholder="この第四形態の特徴や能力を説明してください...">{{ old('description', $fourthFormEvolution->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center">
                            <input type="radio" 
                                   name="evolution_from" 
                                   value="active" 
                                   {{ old('evolution_from', $fourthFormEvolution->evolution_from) === 'active' ? 'checked' : '' }}
                                   required
                        ...
                        <label class="flex items-center">
                            <input type="radio" 
                                   name="evolution_from" 
                                   value="calm" 
                                   {{ old('evolution_from', $fourthFormEvolution->evolution_from) === 'calm' ? 'checked' : '' }}
                                   required
                ...
                <div class="mb-4">
                    ...
                    <select id="evolution_condition_type" 
                        ...
                        <option value="level" {{ old('evolution_condition_type', $fourthFormEvolution->evolution_condition_type) === 'level' ? 'selected' : '' }}>レベル条件</option>
                        <option value="special_item" {{ old('evolution_condition_type', $fourthFormEvolution->evolution_condition_type) === 'special_item' ? 'selected' : '' }}>特別アイテム</option>
                        <option value="weather_condition" {{ old('evolution_condition_type', $fourthFormEvolution->evolution_condition_type) === 'weather_condition' ? 'selected' : '' }}>天気条件</option>
                        <option value="time_condition" {{ old('evolution_condition_type', $fourthFormEvolution->evolution_condition_type) === 'time_condition' ? 'selected' : '' }}>時間条件</option>
                        <option value="friendship" {{ old('evolution_condition_type', $fourthFormEvolution->evolution_condition_type) === 'friendship' ? 'selected' : '' }}>親密度条件</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('evolution_condition_value', $fourthFormEvolution->evolution_condition_value) }}"
                           required
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" 
                           id="sort_order" 
                           name="sort_order" 
                           value="{{ old('sort_order', $fourthFormEvolution->sort_order) }}"
                           min="0"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                           placeholder="0">
                ...
                <div class="mb-4">
                    <label class="flex items-center">
                        <input type="checkbox" 
                               name="is_active" 
                               value="1"
                               {{ old('is_active', $fourthFormEvolution->is_active) ? 'checked' : '' }}
